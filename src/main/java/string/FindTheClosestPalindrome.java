    ...
    public String nearestPalindromic(String n) {
        ...
        if (isPalindrome(n)) {
            ...
            if (n.length() % 2 == 0) {
                ...
                var firstHalf = n.substring(0, n.length() / 2);
                var middle = n.charAt(n.length() / 2);
                if (middle == '0') {
            ...
            if (n.length() % 2 == 0) {
                ...
                var temp = String.valueOf(Long.parseLong(firstHalf) + 1);
                p2 = temp + new StringBuilder(temp).reverse();
                temp = String.valueOf(Long.parseLong(firstHalf) - 1);
                var p3 = temp + new StringBuilder(temp).reverse();
                p1 =
        ...
        var l1 = Math.abs(Long.parseLong(n) - Long.parseLong(p1));
        var l2 = Math.abs(Long.parseLong(n) - Long.parseLong(p2));
        if (l1 <= l2) {
            palindrome = p1;
        } else palindrome = p2;
        var m1 = Math.abs(Long.parseLong(getLow(n)) - Long.parseLong(n));
        var m2 = Math.abs(Long.parseLong(palindrome) - Long.parseLong(n));
        var m3 = Math.abs(Long.parseLong(getHigh(n)) - Long.parseLong(n));
        var min = Math.min(Math.min(m1, m2), m3);
