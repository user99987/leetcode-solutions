...
class SmallestSufficientTeamK {
    ...
    fun smallestSufficientTeam(skills: Array<String>, people: List<List<String>>): IntArray {
        ...
        val personSkills = IntArray(m)
        for (i in 0 until m) {
            var skillMask = 0
            for (skill in people[i]) {
                skillMask = skillMask or (1 shl skillIndex[skill]!!)
            }
            personSkills[i] = skillMask
        }

        val dp = arrayOfNulls<MutableList<Int>>(1 shl n)
        dp[0] = ArrayList()

        for (i in 0 until m) {
            val skillSet = personSkills[i]
            if (skillSet == 0) continue
            for (prev in (1 shl n) - 1 downTo 0) {
                if (dp[prev] == null) continue
                val combined = prev or skillSet
                if (dp[combined] == null || dp[prev]!!.size + 1 < dp[combined]!!.size) {
                    dp[combined] = ArrayList(dp[prev])
                    dp[combined]!!.add(i)
                }
            }
        }
