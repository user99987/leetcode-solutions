 ...
 * Hard
 *
 * A game on an undirected graph is played by Mouse and Cat. Return 1 if the mouse wins, 2 if the
 * cat wins, or 0 if the game is a draw, assuming both play optimally.
 */
class CatAndMouseK {
    ...
    fun catMouseGame(graph: Array<IntArray>): Int {
        val n = graph.size
        val states = Array(n) { Array(n) { IntArray(2) } }
