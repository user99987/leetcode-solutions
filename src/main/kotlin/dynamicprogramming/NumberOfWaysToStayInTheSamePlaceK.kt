 ...
 * 1 <= arrLen <= 10^6
 */
class NumberOfWaysToStayInTheSamePlaceK {

    fun numWays(steps: Int, arrLen: Int): Int {
        val maxPos = minOf(arrLen - 1, steps / 2)
        var current = IntArray(maxPos + 1)
        current[0] = 1

        for (step in 1..steps) {
            val next = IntArray(maxPos + 1)
            val upperBound = minOf(maxPos, step)
            for (position in 0..upperBound) {
                var ways = current[position].toLong()
                if (position > 0) {
