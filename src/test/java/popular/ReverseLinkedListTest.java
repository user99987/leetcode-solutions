package popular;

import org.junit.jupiter.api.Test;
import utils.ListNode;
import static org.assertj.core.api.Assertions.assertThat;


public class ReverseLinkedListTest {

    @Test
    public void shouldReverseLongerList() {
        ListNode input = ListNode.create(new int[]{1, 2, 3, 4, 5});
        ListNode result = new ReverseLinkedList().reverseList(input);
        assertThat(ListNode.toArray(result)).containsExactly(new int[]{5, 4, 3, 2, 1});
    }

    @Test
    public void shouldReverseTwoElementList() {
        ListNode input = ListNode.create(new int[]{1, 2});
        ListNode result = new ReverseLinkedList().reverseList(input);
        assertThat(ListNode.toArray(result)).containsExactly(new int[]{2, 1});
    }

    @Test
    public void shouldReturnEmptyListWhenInputIsEmpty() {
        ListNode input = ListNode.create(new int[]{});
        ListNode result = new ReverseLinkedList().reverseList(input);
        assertThat(ListNode.toArray(result)).containsExactly(new int[]{});
    }
}
